...
legend_labels = ["x-direction", "y-direction", "z-direction"]
...
plot_mag.set_xlabel(f"Field strength \$|h|\$")  # f"Magnetic field in {f_dict[field_directions[i]]}-direction \$h_{f_dict[field_directions[i]]}\$"
plot_mag.set_ylabel("\$ |E_{\\boldsymbol{\\theta}}-E_\\mathrm{exact}| / |E_\\mathrm{exact}| \$ ")
plot_mag.set_yscale("log")
#plot_mag.set_ylim(1e-9, 1e-3)
plot_mag.legend()
fig.savefig(f"{save_dir}/error_comparison_L{shape}_cbdirections_h{f_dict[field_directions[0]]}.svg")