...
def expect_batched(vqs: nk.vqs.MCState,
                   obs: nk.operator.AbstractOperator,
                   n_samples: int,
                   chunk_size: int = 1024):

    assert n_samples % chunk_size == 0, "chunk_size must divide n_samples"
    assert chunk_size % vqs.n_chains_per_rank == 0, "chains_per_rank must divide chunk_size"
    
    n_batches = int(n_samples / chunk_size)
    chains_per_rank = vqs.n_chains_per_rank
    samples_per_chain = int(n_samples / chains_per_rank)
    batch_samples_per_chain = int(chunk_size / chains_per_rank)
    
    loc_ests = jnp.zeros(shape=(chains_per_rank, samples_per_chain), dtype = jnp.complex128)
    for i in tqdm(range(n_batches), desc=f"batch number (chunk_size={chunk_size})"):
        vqs.reset()
        vqs.sample(n_samples=chunk_size)
        loc_ests = loc_ests.at[:, i*batch_samples_per_chain:(i+1)*batch_samples_per_chain].set(vqs.local_estimators(obs))
    return statistics(loc_ests)