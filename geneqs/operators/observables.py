...
# %%
class AbsMagnetization(nk.operator.DiscreteOperator):
    """
    ...
    def get_conn_padded(self, sigma: jax.Array) -> Tuple[jax.Array, jax.Array]:
        ...
        """
        return abs_magnetization_conns_and_mels(sigma)

    def get_conn_flattened(self, x, sections):
        eta, mels = abs_magnetization_conns_and_mels(x)
...
def get_local_kernel(vstate: nk.vqs.MCState, op: AbsMagnetization):
    return e_loc


@nk.vqs.get_local_kernel_arguments.dispatch
def get_local_kernel_arguments(vstate: nk.vqs.MCState, op: AbsMagnetization):
    sigma = vstate.samples
    extra_args = abs_magnetization_conns_and_mels(sigma.reshape(-1, vstate.hilbert.size))
    return sigma, extra_args


@jax.jit
@partial(jax.vmap, in_axes=(0,))
def abs_magnetization_conns_and_mels(sigma: jax.Array) -> Tuple[jax.Array, jax.Array]:
    """
