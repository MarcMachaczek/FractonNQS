...
# %%
sampler = nk.sampler.MetropolisLocal(hilbert, n_chains=1024/2, dtype=jnp.int8)
params = RBMSymm.init(jax.random.PRNGKey(0), hilbert.random_state(jax.random.PRNGKey(1), 3))
sampler_state = sampler.init_state(RBMSymm, params, jax.random.PRNGKey(2))

res, time = geneqs.utils.benchmarking.time_function(sampler.sample, 1, RBMSymm, params, state=sampler_state,
                                                    chain_length=8,)
